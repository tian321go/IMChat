package cn.ipmsgchat.android.activity.message;
...
import cn.ipmsgchat.android.adapter.ChatAdapter;
import cn.ipmsgchat.android.entity.Message;
import cn.ipmsgchat.android.entity.Users;
import cn.ipmsgchat.android.socket.tcp.TcpClient;
import cn.ipmsgchat.android.socket.udp.IPMSGConst;
import cn.ipmsgchat.android.socket.udp.OnActiveChatActivityListenner;
import cn.ipmsgchat.android.sql.SqlDBOperate;
import cn.ipmsgchat.android.util.AudioRecorderUtils;
...
import cn.ipmsgchat.android.view.ChatListView;
import cn.ipmsgchat.android.view.EmoteInputView;
import cn.ipmsgchat.android.view.EmoticonsEditText;
import cn.ipmsgchat.android.view.HeaderLayout;
import cn.ipmsgchat.android.view.ScrollLayout;
...
public class ChatActivity extends BaseMessageActivity implements OnActiveChatActivityListenner {
    ...
    protected void initViews() {
        ...
        mHeaderLayout.init(HeaderLayout.HeaderStyle.TITLE_CHAT);
    ...
    private void init() {
        ...
        mMessagesList = mDBOperate.getScrollMessageOfChattingInfo(0, 105, mSenderID, mID);
                ...
                if (recordState == RECORD_ON) {
                    ...
                    if (!isMove) {
                        if (recodeTime < MIN_RECORD_TIME) {
                            ...
                            sendMessage(mVoicePath, Message.CONTENT_TYPE.VOICE);
                            refreshAdapter();
                        }
                    }
        ...
        switch (v.getId()) {
            ...
            case R.id.chat_textditor_btn_send:
                ...
                if (!TextUtils.isEmpty(content)) {
                    ...
                    sendMessage(content, Message.CONTENT_TYPE.TEXT);
                    refreshAdapter();
                }
                break;
        ...
        switch (requestCode) {
                            ...
                            if (ImageUtils.bitmapIsLarge(bitmap)) {
                                ...
                                if (path != null) {
                                    ImageUtils.createThumbnail(this, path, THUMBNAIL_PATH
                                            + File.separator);
                                    sendMessage(path, Message.CONTENT_TYPE.IMAGE);
                                    refreshAdapter();
                                }
                            }
                        }
            ...
            case FILE_SELECT_CODE: {
                if (resultCode == RESULT_OK) {
                    ...
                    if (path != null) {
                        sendFilePath = path;
                        sendMessage(sendFilePath, Message.CONTENT_TYPE.FILE);
                        refreshAdapter();
                    }
                }
            }
    ...
    private void startRecord() {
        ...
        RECORD_FILENAME = System.currentTimeMillis()
                + cn.ipmsgchat.android.util.TextUtils.getRandomNumStr(3);
