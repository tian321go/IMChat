...
public class SqlDBOperate
{
        ...
        if (cursor.moveToNext()) {
            UserInfo userInfo = new UserInfo(cursor.getInt(cursor.getColumnIndex("id")),
                    ...
                    cursor.getString(cursor.getColumnIndex("ip")), cursor.getInt(cursor
                            ...
                            cursor.getString(cursor.getColumnIndex("device")));
            cursor.close();
            return userInfo;
        }
        cursor.close();
    ...
    public List<UserInfo> getScrollDataOfUserInfo(int start, int count) {
        ...
        while (cursor.moveToNext()) {
            users.add(new UserInfo(cursor.getInt(cursor.getColumnIndex("id")), cursor
                    .getString(cursor.getColumnIndex("name")), cursor.getString(cursor.getColumnIndex("sex")), cursor
                    .getString(cursor.getColumnIndex("IMEI")), cursor.getString(cursor
                    .getColumnIndex("ip")), cursor
     ...
     * 参数：chattinginfo类 作用：用来添加聊天记录
     */
    public void addChattingInfo(int senderID,int recieverID,String time,String content,Message.CONTENT_TYPE type) {
    ...
    private int getStyteByContentType(Message.CONTENT_TYPE type)
    {
    	if(type== Message.CONTENT_TYPE.TEXT)
    	{
    		return 0;
    	}else if(type== Message.CONTENT_TYPE.IMAGE)
    	{
    		return 1;
    	}else if (type== Message.CONTENT_TYPE.FILE)
		{
			return 2;
		}else if(type== Message.CONTENT_TYPE.VOICE)
		{
			return 3;
		}
    	return -1;
     ...
     * 参数：chattinginfo类 作用：用来添加聊天记录
     */
    public void addChattingInfo(String senderIMEI,String recieverIMEI,String time,String content,Message.CONTENT_TYPE type) {
